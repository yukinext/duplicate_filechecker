...
- [x] 2.4 (P) Merger コンポーネントの実装
  ...
  - ディレクトリ構造を維持した移動を実装
  - 移動先ディレクトリが存在しない場合は作成し、既に存在する場合はそのまま利用する
  - 同名ファイルが移動先に存在する場合はログ出力してファイル名に `_1` などを付与し重複を避ける
  - 統合スイッチのデフォルトoffを実装
  - ファイル移動が正常に行われることを確認
...
- [x] 3.1 CLI 引数解析の実装
  ...
  - trash_dir のデフォルト値を探索するディレクトリのパスに `.dup_trash` を付与したパスに設定
  - 引数解析が正常に行われることを確認
...
- [ ] 6. 要件追加への対応
- [x] 6.1 Logger にスキップログ出力機能を追加
  - ファイルがDBキャッシュによりスキップされた場合にログを出力する機能を実装
  - ログメッセージにスキップされたファイルパスを含める
  - ログ出力が正常に行われることを確認
  - _Requirements: 6.5_
  - _Boundary: Logger_

- [x] 6.2 Logger に移動ログ出力機能を追加
  - 枝ファイルが移動された場合にログを出力する機能を実装
  - ログメッセージに移動されたファイルパスを含める
  - ログ出力が正常に行われることを確認
  - _Requirements: 6.6_
  - _Boundary: Logger_

- [x] 6.3 Logger に処理時間ログ出力機能を追加
  - 処理の終了時に処理時間をログに出力する機能を実装
  - 処理時間を秒単位で計算して表示
  - ログ出力が正常に行われることを確認
  - _Requirements: 6.7_
  - _Boundary: Logger_

- [x] 6.4 CLI に処理時間計測機能を追加
  - 処理開始時に時間を記録する機能を実装
  - 処理終了時に時間を計算してLoggerに渡す機能を実装
  - 処理時間が正確に計測されることを確認
  - _Requirements: 6.7_
  - _Boundary: CLI_

- [x] 6.5 Logger のスキップログに幹ファイルパスを追加
